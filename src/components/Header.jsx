...
const NavIteminfo = [
  ...
  {name: "Pages", type: "dropdown", items: ["About us", "Contact us"] }, //19min on 4th video
...
<span className='text-blue-500 absolute transition-all duration-500 font-bold right-0 top-0 group-hover:right-[90%] opacity-0 group-hover:opacity-100'>
  ...
  <div className='hidden transition-all duration-500 pt-4 absolute bottom-0 right-0 transform translate-y-full group-hover:block w-max'>
    <ul className='flex flex-col shadow-lg rounded-lg overflow-hidden'>
      {item.items.map((page) => (
        <a href="/" className='hover:bg-dark-hard  px-4 py-2 text-dark-soft lg:text-dark-soft'>
          {page}
        </a>
      ))}
    </ul>
  ...
  return <section>
    <header className="container mx-auto px-5 flex justify-between py-4 items-center">
        ...
        <div className={`${navIsVisible ? "right-0" : "-right-full"} transition-all duration-300 mt-[56px] lg:mt-0 bg-dark-hard lg:bg-transparent z-[49] flex flex-col w-full lg:w-auto justify-center lg:justify-end lg:flex-row fixed top-0 bottom-0 lg:static gap-x-9 items-center`}>
          <ul className='text-dark-soft items-center gap-y-5 lg:bg-white flex flex-col lg:flex-row gap-x-2 font-semibold'>
